...
const DetailOrderOnProcessModal = ({isOpen, onClose, data}) => {
	...
	const finish = (id_order) => {
		setLoading(true)
		reqFinish(id_order).then(result => {
			setAlert({isOpen: true, status: 'success', message: 'Pesanan selesai. Tunggu kurir untuk mengambil pesanan!'})
			onClose()
		}).catch(error => {
			if(error.status === 500){
	...
	return (
		...
		<button className={d.btnReject} disabled={loading}>TOLAK</button>
		<button className='btn-primary' onClick={() => {finish(data.id_order)}} disabled={loading}>SELESAI</button>
		</div>
		</div>
		</div>
		</>
...
export default DetailOrderOnProcessModal